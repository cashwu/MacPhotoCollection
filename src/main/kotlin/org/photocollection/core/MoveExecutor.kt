...
object MoveExecutor {
    ...
    /**
     * Execute [plan]'s moves item by item. Each item creates its nested year/month/date (or
     * `no-exif`) subfolder idempotently with `Files.createDirectories`, which creates every missing
     * parent in the path, then moves the file with `Files.move` invoked with no replace-existing
     * option, so an already-occupied target throws and is caught as a conflict. One item's failure
     * never aborts the rest, and a failed item leaves its source file unchanged.
     */
