...
class PlanResultViewTest {
    ...
    fun `shows each move's target folder with its date source and the per-source summary`() = runComposeUiTest {
        ...
        val shotTarget = Paths.get("/photos/2025/2025-10-28/SHOT.png")
        ...
        // Each row shows the target leaf folder and its date source; EXIF and FILENAME differ, the
        // year-only row is marked EXIF, and the no-date row carries no source marker.
        onNodeWithText("FULL.jpg  →  2024-03-15  [EXIF]").assertIsDisplayed()
        onNodeWithText("SHOT.png  →  2025-10-28  [FILENAME]").assertIsDisplayed()
        onNodeWithText("YEAR.png  →  2008-00-00  [EXIF]").assertIsDisplayed()
